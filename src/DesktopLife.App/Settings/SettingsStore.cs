...
public sealed class SettingsStore(string? filePath = null)
{
    ...
    public AppPreferences LoadPreferences(out bool invalid)
    {
        ...
        try
        {
            ...
            if (!Enum.IsDefined(value.Style) || value.StartHotkey == null || value.PauseHotkey == null || !DesktopLife.Rendering.Localization.UiLanguage.IsSupported(value.Language) || !Hotkey.TryParse(value.StartHotkey, out var a) || !Hotkey.TryParse(value.PauseHotkey, out var b) || (a.Key != 0 && a == b)) throw new JsonException();
            return value;
        }
