...
v0.4.2 的桌面与屏保共用 13 种生物的模拟与绘制（12 种昆虫及蜘蛛）。全桌面固定一只苍蝇，其余物种按全桌面总数配置；相接的屏幕可连续通行。

## 新增物种

| 物种 | 100% 身体长度（物理像素，不含足、触角及尾部附肢） | 外观与动作 |
| --- | ---: | --- |
| 瓢虫 Ladybug | 10 | 红黑圆拱鞘翅，短触角，小步爬行与展翅飞行 |
| 步甲 Ground beetle | 22 | 深色纵纹鞘翅，较快爬行，接近鼠标时躲避 |
| 蠼螋 Earwig | 24 | 分节腹部与尾钳，爬行和短暂停步 |
| 衣鱼 Silverfish | 18 | 银灰渐尖身体，三根尾丝，快速转向 |
| 蟋蟀 Cricket | 26 | 深褐身体、长触角、粗后腿，行走、蓄力跳跃 |
...
蜘蛛属于蛛形纲，采用地栖狼蛛类外形作为参考，四对步足从头胸部伸出，没有触角。写实身体使用 AI 生成纹理，八足及短触肢由程序绘制；可爱版使用淡紫色圆润身体。两种外观都支持尺寸混合、跨屏爬行和屏保。v0.4.2 不模拟结网、捕食或跳跃；当前源码及本地 0.4.3-preview.1 另加入趣味化的“射丝 → 牵引到落点 → 丝网淡出”逃生动作，详见[动作说明](LOCOMOTION.md)。形态参考：[马里兰大学蜘蛛介绍](https://www.extension.umd.edu/resource/predatory-spiders)。
...
爬行步态由实际移动距离驱动，停步时保持足部姿势。各类拥有不同速度、转向速度与停步概率。蟋蟀与蚱蜢已支持跳跃，瓢虫已支持起飞、飞行和落地收翅，两种外观共用运动状态。暂不模拟蚱蜢飞行、捕食和蜕皮。[动作说明](LOCOMOTION.md)。
