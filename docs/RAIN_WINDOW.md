...
水痕宽度随雨滴半径增加，初始宽度最大 11 像素，渲染时随年龄逐渐收细、淡出。宽水痕保持连贯，用低透明度水膜及细边缘反光表现，不再统一压缩成细亮线。

修复宽水痕的黑色圆圈：水痕线段采用平头衔接，避免圆形端点重叠叠色；宽度和透明度变化放缓。诊断增加白底水痕接缝的像素检查。
