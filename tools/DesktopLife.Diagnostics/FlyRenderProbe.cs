...
internal static class FlyRenderProbe
{
    public static void Run(string output)
    {
        ...
        foreach (var scale in new[] { 1.0, 1.25, 1.5 })
        {
            ...
            if (a.Where((v, i) => i % 4 == 3 && v > 10).Count() > 3000) throw new Exception("Sprite unexpectedly fills its texture rectangle");
        ...
        return;
        RenderTargetBitmap Render(bool resting, double scale)
        {
            var visual = new DrawingVisual();
            using (var dc = visual.RenderOpen())
                renderer.Render(dc, [new PoseCreature(new(-920, 80), resting)], new(-1000, 0, 160, 160), 0.1f, scale, scale);
    ...
    private sealed class PoseCreature : Creature
    {
        ...
        public PoseCreature(Vector2 position, bool resting) { Position = position; _resting = resting; }
